      ...
      <Page size="A4" style={s.container}>
        <View style={s.section}>
          ...
          <View style={s.flexRow}>
            {nulledResume.about.photo ? (
              <Image style={s.photo} src={nulledResume.about.photo} />
            ) : null}
        ...
        <View style={s.section}>
          ...
          {nulledResume.projects.map((item, index) => (
            <View style={s.article} key={index}>
              ...
              {item.startDate || item.endDate || item.currentlyWork ? (
                <View style={s.dates}>
                  <Text>{item.startDate}</Text>
                  {(item.currentlyWork || item.endDate) && <Text>-</Text>}
                  <Text>
                    {item.currentlyWork
              ...
              <View>
                {item.technologies && (
                  <Text style={{ marginBottom: 10 }}>{item.technologies}</Text>
                )}
              ...
              {item.deployUrl || item.sourceUrl ? (
                <View style={s.flexRow}>
                  {item.deployUrl ? (
                    <Link src={item.deployUrl}>
                      {titles.projects.deployUrl}
                    </Link>
                  ) : null}

                  {item.sourceUrl ? (
                    <Link src={item.sourceUrl}>
                      {" "}
                      {titles.projects.sourceUrl}
                    </Link>
                  ) : null}
                </View>
              ) : null}
          ...
          {nulledResume.experiences.map((item, index) => (
            <View style={s.article} key={index}>
              ...
              <View style={{ ...s.flexRow, marginBottom: 10 }}>
                ...
                <View style={s.dates}>
                  <Text>{item.startDate !== null ? item.startDate : null}</Text>
                  {item.currentlyWork ? (
                    <>
                      <Text>-</Text>
                    ...
                    <>
                      <Text>-</Text>
                      <Text>{item.endDate !== null ? item.endDate : null}</Text>
                    </>
                  )}
                </View>
              </View>
        ...
        <View style={s.section}>
          ...
          {nulledResume.educations.map((item, index) => (
            <View style={s.article} key={index}>
              ...
              {item.activities && (
                <>
                  <Text>{titles.educations.activities}</Text>
